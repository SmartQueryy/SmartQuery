...
from models.response_schemas import (
    ApiResponse, ChatMessage, SendMessageRequest, SendMessageResponse,
    QueryResult, PaginatedResponse, CSVPreview, QuerySuggestion
)
...
MOCK_CSV_PREVIEWS = {
    "project_001": {
        "columns": ["date", "product_name", "sales_amount", "quantity", "category", "region", "customer_id", "discount"],
        "sample_data": [
            ["2024-01-01", "Product A", 1500.00, 10, "Electronics", "North", "CUST001", 0.1],
            ["2024-01-02", "Product B", 2300.50, 15, "Clothing", "South", "CUST002", 0.05],
            ["2024-01-03", "Product C", 1890.25, 12, "Electronics", "East", "CUST003", 0.15],
            ["2024-01-04", "Product A", 1200.00, 8, "Electronics", "West", "CUST004", 0.0],
    ...
    "project_002": {
        "columns": ["customer_id", "age", "city", "signup_date", "total_orders", "lifetime_value"],
        "sample_data": [
...
def generate_mock_query_result(query: str, project_id: str) -> QueryResult:
    ...
    if "total sales" in query.lower() or "sum" in query.lower():
        ...
        sql_query = "SELECT region, AVG(sales_amount) as avg_sales FROM data GROUP BY region"
        result_data = [
        ...
        result_data = [
            {"date": "2024-01-01", "product_name": "Product A", "sales_amount": 1500.00},
            {"date": "2024-01-02", "product_name": "Product B", "sales_amount": 2300.50},
            {"date": "2024-01-03", "product_name": "Product C", "sales_amount": 1890.25}
        ]
        result_type = "table"
    ...
    return QueryResult(
        ...
        } if result_type == "chart" else None
    )
