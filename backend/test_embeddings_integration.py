...
def test_embeddings_integration():
    ...
    with patch.object(embeddings_service, 'get_embedding_stats') as mock_stats, \
         patch.object(embeddings_service, 'generate_project_embeddings') as mock_generate, \
        ...
        mock_search.assert_called_once_with(project_id, user_id, "Tell me about customer data", top_k=3)
        
        # Verify result contains semantic information
        assert result.result_type == "summary"
        assert "sales dataset" in result.summary.lower() or "customer" in result.summary.lower()
    ...
    col_desc = embeddings_service._create_column_description(mock_project.columns_metadata[0])
