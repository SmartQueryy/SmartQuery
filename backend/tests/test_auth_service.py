...
class TestAuthService:
    ...
    def test_create_access_token(self, auth_service):
        ...
        payload = jwt.decode(token, auth_service.jwt_secret, algorithms=[auth_service.algorithm])
    ...
    def test_verify_expired_token(self, auth_service):
        ...
        expired_token = jwt.encode(to_encode, auth_service.jwt_secret, algorithm=auth_service.algorithm)
    ...
    def test_verify_google_token_production_no_config(self, auth_service):
        ...
        with patch.object(auth_service, 'google_client_id', None):
            with patch.object(auth_service, 'environment', 'production'):
                with pytest.raises(ValueError, match="Google OAuth is not properly configured for production"):
                    auth_service.verify_google_token("real_google_token")
    ...
    @patch('services.auth_service.id_token.verify_oauth2_token')
    def test_verify_production_google_token_missing_fields(self, mock_verify, auth_service):
        ...
        with patch.object(auth_service, 'google_client_id', 'real_client_id'):
            with patch.object(auth_service, 'enable_mock_auth', False):
                with pytest.raises(ValueError, match="Missing required Google OAuth fields"):
                    auth_service.verify_google_token("real_google_token")

    @patch('services.auth_service.id_token.verify_oauth2_token')
    def test_verify_production_google_token_unverified_email(self, mock_verify, auth_service):
    ...
    def test_validate_google_client_configuration_production_ready(self, auth_service):
        ...
        with patch.object(auth_service, 'environment', 'production'):
            with patch.object(auth_service, 'google_client_id', 'prod_client_id'):
                with patch.object(auth_service, 'google_client_secret', 'prod_client_secret'):
                    config = auth_service.validate_google_client_configuration()
                    
                    assert config['production_ready'] is True
                    assert config['environment'] == 'production'

    def test_validate_google_client_configuration_production_not_ready(self, auth_service):
        ...
        with patch.object(auth_service, 'environment', 'production'):
            with patch.object(auth_service, 'google_client_id', None):
                with patch.object(auth_service, 'google_client_secret', None):
                    ...
                    assert 'Google OAuth credentials not configured for production' in config['issues']
    ...
    def test_enhanced_health_check_healthy(self, auth_service, sample_user):
        ...
        with patch.object(auth_service.user_service, 'health_check', return_value=mock_user_health):
    ...
    def test_enhanced_health_check_unhealthy_user_service(self, auth_service):
        ...
        with patch.object(auth_service.user_service, 'health_check', side_effect=Exception("DB connection failed")):
            health = auth_service.health_check()
    ...
    def test_mock_token_user_id_extraction(self, auth_service):
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "development", "ENABLE_MOCK_AUTH": "true"}):
            with patch.object(auth_service, 'google_client_id', 'mock_client_id'):
    ...
    def test_login_with_google_success(self, auth_service, google_oauth_data, sample_user):
        """Test successful Google login"""
        with patch.object(auth_service, 'verify_google_token', return_value=google_oauth_data):
            with patch.object(auth_service.user_service, 'create_or_update_from_google_oauth', return_value=(sample_user, True)):
                with patch.object(auth_service.user_service, 'update_last_sign_in', return_value=sample_user):
                    
                    user, access_token, refresh_token, is_new = auth_service.login_with_google("mock_token")
    ...
    def test_refresh_access_token_success(self, auth_service, sample_user):
        ...
        refresh_token = auth_service.create_refresh_token(str(sample_user.id), sample_user.email)
    ...
    def test_refresh_access_token_user_not_found(self, auth_service):
        ...
        refresh_token = auth_service.create_refresh_token("nonexistent_user", "nonexistent@example.com")
    ...
    def test_refresh_access_token_inactive_user(self, auth_service, sample_user):
        ...
        with patch.object(auth_service.user_service, 'get_user_by_email', return_value=sample_user):
            with pytest.raises(jwt.InvalidTokenError, match="User account is deactivated"):
                auth_service.refresh_access_token(refresh_token)

    def test_get_current_user_success(self, auth_service, sample_user):
        """Test successful current user retrieval"""
        access_token = auth_service.create_access_token(str(sample_user.id), sample_user.email)
