...
class TestAuthIntegration:
    ...
    def expired_token(self, sample_user):
        ...
        return jwt.encode(payload, auth_service.jwt_secret, algorithm=auth_service.algorithm)

    def test_google_oauth_login_success(self, sample_user, google_oauth_data):
        """Test successful Google OAuth login flow"""
        with patch('api.auth.auth_service.verify_google_token', return_value=google_oauth_data):
            with patch('api.auth.auth_service.user_service.create_or_update_from_google_oauth', return_value=(sample_user, True)):
                with patch('api.auth.auth_service.user_service.update_last_sign_in', return_value=sample_user):
    ...
    def test_google_oauth_login_invalid_token(self):
        ...
        response = client.post(
            "/auth/google",
            json={"google_token": "invalid_token_123"}
        )
    ...
    def test_refresh_token_success(self, sample_user, valid_refresh_token):
        ...
        with patch('api.auth.auth_service.refresh_access_token', return_value=(valid_refresh_token, sample_user)):
...
class TestAuthMiddlewareIntegration:
    ...
    def test_middleware_no_authorization_header(self):
        ...
        assert response.status_code == 403  # FastAPI security returns 403 for missing header
...
class TestAPIResponseFormat:
    ...
    def test_success_response_format(self, sample_user):
        ...
        with patch('api.auth.auth_service.login_with_google') as mock_login:
            mock_login.return_value = (sample_user, "access_token", "refresh_token", True)
    ...
    def test_user_data_format(self, sample_user):
        ...
        with patch('api.auth.auth_service.login_with_google') as mock_login:
            ...
            assert user_data["avatar_url"] is None or isinstance(user_data["avatar_url"], str)
    ...
    def test_token_data_format(self, sample_user):
        ...
        with patch('api.auth.auth_service.login_with_google') as mock_login:
            mock_login.return_value = (sample_user, "test_access_token", "test_refresh_token", True)
...
class TestErrorHandling:
    ...
    def test_google_oauth_service_error(self):
        ...
        with patch('api.auth.auth_service.verify_google_token', side_effect=Exception("Google service unavailable")):
    ...
    def test_database_error_handling(self, sample_user):
        ...
        with patch('api.auth.auth_service.verify_google_token', return_value=google_oauth_data):
            with patch('api.auth.auth_service.user_service.create_or_update_from_google_oauth', side_effect=Exception("Database connection failed")):
    ...
    def test_jwt_service_error_handling(self):
        ...
        with patch('middleware.auth_middleware.auth_service.verify_token', side_effect=Exception("JWT service error")):
