...
from services.langchain_service import langchain_service, LangChainService
...
class TestLangChainChatIntegration:
    ...
    def test_sql_query_processing(
        self, test_client, test_access_token, test_user_in_db, test_project_with_metadata
    ):
    ...
    def test_error_handling_with_fallback(
        ...
        with patch("services.langchain_service.langchain_service") as mock_service:
            # Mock service error
            mock_service.process_query.side_effect = Exception("LangChain service unavailable")
        ...
        for case in test_cases:
            with patch("services.langchain_service.langchain_service") as mock_service:
                mock_result = Mock(
                    ...
                    chart_config={"type": "bar"} if case["result_type"] == "chart" else None,
                    summary="This is a summary response" if case["result_type"] == "summary" else None,
                    error="Test error message" if case["result_type"] == "error" else None,
                    sql_query="SELECT * FROM data" if case["result_type"] in ["table", "chart"] else None,
                )
...
class TestLangChainServiceUnit:
    ...
    def test_sql_generation_tool(self, mock_chat_openai):
        ...
        mock_llm = Mock()
        mock_response = Mock()
        mock_response.content = "SELECT product_name, SUM(sales_amount) FROM data GROUP BY product_name"
