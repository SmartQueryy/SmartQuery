...
class TestAuthMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_optional_success(self, auth_middleware, sample_user, valid_credentials):
        """Test optional user retrieval with valid token"""
        with patch.object(auth_middleware.auth_service, 'get_current_user', return_value=sample_user):
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_optional_invalid_token(self, auth_middleware, invalid_credentials):
        """Test optional user retrieval with invalid token"""
        with patch.object(auth_middleware.auth_service, 'get_current_user', side_effect=jwt.InvalidTokenError("Invalid token")):
            user = await auth_middleware.get_current_user_optional(invalid_credentials)
            assert user is None

    @pytest.mark.asyncio
    async def test_get_current_user_required_success(self, auth_middleware, sample_user, valid_credentials):
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_required_invalid_token(self, auth_middleware, invalid_credentials):
    ...
    async def test_verify_token_only_success(self, auth_middleware, valid_credentials):
        ...
        with patch.object(auth_middleware.auth_service, 'verify_token', return_value=mock_token_data):
    ...
    @pytest.mark.asyncio
    async def test_verify_token_only_invalid_token(self, auth_middleware, invalid_credentials):
        """Test token verification with invalid token"""
        with patch.object(auth_middleware.auth_service, 'verify_token', side_effect=jwt.InvalidTokenError("Invalid token")):
            with pytest.raises(HTTPException) as exc_info:
...
class TestAuthDependencies:
    ...
    async def test_get_current_user_dependency_success(self, sample_user):
        ...
        with patch('middleware.auth_middleware.auth_middleware') as mock_middleware:
            mock_middleware.get_current_user_required = AsyncMock(return_value=sample_user)
    ...
    async def test_get_current_user_optional_dependency(self, sample_user):
        ...
        with patch('middleware.auth_middleware.auth_middleware') as mock_middleware:
            mock_middleware.get_current_user_optional = AsyncMock(return_value=sample_user)
