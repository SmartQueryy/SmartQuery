...
class TestLangChainIntegration:
    ...
    def test_langchain_query_processing_integration(self, mock_openai, test_db_setup):
        ...
        project_data = ProjectCreate(
            ...
            description="Testing LangChain integration"
        )
        ...
        with patch('services.langchain_service.DuckDBService') as mock_duckdb_class:
            ...
            result = langchain_service.process_query(test_project.id, "Show me all the data")
    ...
    def test_embeddings_integration(self, mock_openai, test_db_setup):
        ...
        project_data = ProjectCreate(
            ...
            description="Testing embeddings integration"
        )
        test_project = project_service.create_project(project_data, test_user.id)
    ...
    def test_suggestions_service_integration(self, test_db_setup):
        ...
        for suggestion in suggestions[:3]:  # Check first few suggestions
            ...
            assert suggestion["category"] in ["analysis", "visualization", "summary", "filter"]
    ...
    def test_duckdb_integration(self, mock_duckdb_class, test_db_setup):
        ...
        # Create test user and project
        google_data = GoogleOAuthData(
            google_id="duckdb_test_1",
            email="duckdb@test.com",
            name="DuckDB Test User"
        )
        test_user = user_service.create_user_from_google(google_data)
        
        project_data = ProjectCreate(
            name="DuckDB Test Project",
            description="Testing DuckDB integration"
        )
        test_project = project_service.create_project(project_data, test_user.id)
        ...
        with patch('services.storage_service.storage_service.download_file') as mock_download:
            mock_download.return_value = b"product,sales\nWidget A,1000\nWidget B,1500\nWidget C,2000"
    ...
    @patch('services.langchain_service.OpenAI')
    @patch('services.embeddings_service.OpenAI')
    def test_end_to_end_ai_workflow(self, mock_embeddings_openai, mock_langchain_openai, test_db_setup):
        ...
        mock_embeddings_client.embeddings.create.return_value.data = [Mock(embedding=[0.1] * 1536)]
        ...
        project_data = ProjectCreate(
            ...
            description="Testing end-to-end AI workflow"
        )
        test_project = project_service.create_project(project_data, test_user.id)
