...
class TestWorkflowIntegration:
    ...
    def test_complete_user_authentication_workflow(self, test_db_setup):
        ...
        with patch('services.auth_service.AuthService.validate_google_token') as mock_validate:
            mock_validate.return_value = google_data
            
            # Step 2: Login and get tokens
            user, access_token, refresh_token, is_new_user = auth_service.login_with_google("mock_token")
    ...
    def test_complete_project_lifecycle_workflow(self, test_db_setup):
        ...
        try:
            ...
            response = client.post(
                "/projects",
                json={"name": "Workflow Test Project", "description": "Complete workflow test"},
    ...
    def test_complete_chat_workflow(self, mock_duckdb_class, mock_openai, test_db_setup):
        ...
        # Mock services
        mock_llm = Mock()
        mock_llm.invoke.return_value.content = "SELECT name, age FROM data WHERE age > 25"
        ...
        project_data = ProjectCreate(
            name="Chat Workflow Project",
            description="Testing complete chat workflow"
        )
    ...
    def test_multi_user_workflow_isolation(self, test_db_setup):
        ...
        user1_data = GoogleOAuthData(
            google_id="isolation_user_1",
            email="user1@test.com",
            name="User One"
        )
        user2_data = GoogleOAuthData(
            google_id="isolation_user_2",
    ...
    def test_error_recovery_workflow(self, test_db_setup):
        ...
        try:
            ...
            response = client.post(
                "/projects",
                json={"name": "Recovery Test Project", "description": "Testing error recovery"},
                headers={"Authorization": "Bearer mock_token"}
            )
            assert response.status_code == 200
            data = response.json()
