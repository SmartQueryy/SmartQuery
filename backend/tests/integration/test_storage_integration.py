...
class TestStorageIntegration:
    ...
    def test_presigned_url_generation_integration(self, test_db_setup):
        ...
        project_data = ProjectCreate(
            name="Storage Test Project", description="Testing storage integration"
        )
    ...
    def test_file_upload_download_cycle_integration(
        self, mock_download, mock_upload, test_db_setup
    ):
        ...
        # Create test user and project
        google_data = GoogleOAuthData(
            google_id="upload_test_1", email="upload@test.com", name="Upload Test User"
        )
        test_user, _ = user_service.create_or_update_from_google_oauth(google_data)

        project_data = ProjectCreate(
            name="Upload Test Project", description="Testing file upload integration"
        )
        test_project = project_service.create_project(project_data, test_user.id)
    ...
    def test_file_metadata_integration(self, mock_metadata, test_db_setup):
        ...
        project_data = ProjectCreate(
            name="Metadata Test Project", description="Testing metadata integration"
        )
        test_project = project_service.create_project(project_data, test_user.id)
    ...
    def test_storage_error_handling_integration(self, test_db_setup):
        ...
        # Create test user and project
        google_data = GoogleOAuthData(
            google_id="error_test", email="error@test.com", name="Error Test User"
        )
        test_user, _ = user_service.create_or_update_from_google_oauth(google_data)
        ...
        with patch(
            "services.storage_service.storage_service.download_file", return_value=None
        ):
            downloaded_content = storage_service.download_file(object_name)
            assert downloaded_content is None  # Should handle missing file gracefully

        with patch(
            "services.storage_service.storage_service.file_exists", return_value=False
        ):
            file_exists = storage_service.file_exists(object_name)
