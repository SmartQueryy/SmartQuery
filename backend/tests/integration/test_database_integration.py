...
class TestDatabaseIntegration:
    ...
    def test_user_lifecycle_integration(self, test_db_setup):
        ...
        created_user = user_service.create_user_from_google(google_data)
    ...
    def test_project_lifecycle_integration(self, test_db_setup):
        ...
        user_projects = project_service.get_projects_by_user(test_user.id, skip=0, limit=10)
        ...
        ownership = project_service.check_project_ownership(created_project.id, test_user.id)
        assert ownership is True
        
        # Test ownership check with wrong user
        wrong_user_id = uuid.uuid4()
        wrong_ownership = project_service.check_project_ownership(created_project.id, wrong_user_id)
        assert wrong_ownership is False
        
        # Test project status update
        updated_project = project_service.update_project_status(created_project.id, "ready")
    ...
    def test_user_project_relationship_integration(self, test_db_setup):
        ...
        user1_data = GoogleOAuthData(
            google_id="rel_test_1",
            email="user1@test.com",
            name="User One"
        )
        user2_data = GoogleOAuthData(
            google_id="rel_test_2", 
            email="user2@test.com",
            name="User Two"
        )
        
        user1 = user_service.create_user_from_google(user1_data)
        user2 = user_service.create_user_from_google(user2_data)
        
        # Create projects for each user
        project1_data = ProjectCreate(name="User 1 Project", description="Project for user 1")
        project2_data = ProjectCreate(name="User 2 Project", description="Project for user 2") 
        project3_data = ProjectCreate(name="User 1 Project 2", description="Second project for user 1")
        ...
        user1_projects = project_service.get_projects_by_user(user1.id, skip=0, limit=10)
        user2_projects = project_service.get_projects_by_user(user2.id, skip=0, limit=10)
    ...
    def test_database_connection_handling(self, test_db_setup):
        ...
        for i in range(5):
            ...
            user = user_service.create_user_from_google(google_data)
    ...
    def test_database_constraints_and_validation(self, test_db_setup):
        ...
        user1 = user_service.create_user_from_google(google_data1)
        ...
        user2 = user_service.create_user_from_google(google_data2)
