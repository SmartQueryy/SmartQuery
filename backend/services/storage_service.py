...
class StorageService:
    ...
    def generate_presigned_url(self, object_name: str, expiry_seconds: int = 3600) -> Optional[str]:
        """Generate presigned URL for file upload"""
        try:
            client = self.get_client()
            url = client.presigned_put_object(self.bucket_name, object_name, expires=expiry_seconds)
            return url
        except Exception as e:
