...
class AuthService:
    ...
    def verify_token(self, token: str, token_type: str = "access") -> TokenData:
        ...
        try:
            ...
            return TokenData(
                ...
                exp=datetime.utcfromtimestamp(exp_timestamp) if exp_timestamp else datetime.utcnow()
            )
        ...
        try:
            ...
            if not self.google_client_id:
                if self.environment == "production":
                    raise ValueError("Google OAuth is not properly configured for production")
                logger.warning("Google Client ID not configured - using development mode")
    ...
    def _is_mock_token(self, token: str) -> bool:
        ...
        return (
            self.enable_mock_auth and 
            self.environment == "development" and 
            token.startswith("mock_google_token")
        )
    ...
    def _handle_mock_token(self, token: str) -> GoogleOAuthData:
        ...
        mock_user_id = token.replace("mock_google_token_", "").replace("mock_google_token", "123")
    ...
    def _verify_production_google_token(self, token: str) -> GoogleOAuthData:
        ...
        try:
            ...
            missing_fields = [field for field in required_fields if not idinfo.get(field)]
            if missing_fields:
                raise ValueError(f"Missing required Google OAuth fields: {missing_fields}")
            
            # Additional security checks
            if not idinfo.get("email_verified", False):
                logger.warning(f"Unverified email from Google OAuth: {idinfo.get('email')}")
            ...
            logger.info(f"Successfully verified Google token for user: {google_data.email}")
            return google_data
    ...
    def login_with_google(self, google_token: str) -> Tuple[UserInDB, str, str, bool]:
        ...
        try:
            ...
            user, is_new = self.user_service.create_or_update_from_google_oauth(google_data)
    ...
    def refresh_access_token(self, refresh_token: str) -> Tuple[str, UserInDB]:
        ...
        try:
            ...
            if not user:
                logger.warning(f"Token refresh failed: User not found for email {token_data.email}")
                raise jwt.InvalidTokenError("User not found")
            
            if not user.is_active:
                logger.warning(f"Token refresh failed: User account inactive {user.email}")
    ...
    def get_current_user(self, access_token: str) -> UserInDB:
        ...
        try:
            ...
            user = self.user_service.get_user_by_email(token_data.email)
            if not user:
                logger.warning(f"Current user request failed: User not found for email {token_data.email}")
                raise jwt.InvalidTokenError("User not found")
            
            if not user.is_active:
                logger.warning(f"Current user request failed: User account inactive {user.email}")
                raise jwt.InvalidTokenError("User account is deactivated")
    ...
    def validate_google_client_configuration(self) -> Dict[str, any]:
        ...
        if self.environment == "production":
            if not self.google_client_id or not self.google_client_secret:
                ...
                config_status["issues"] = ["Google OAuth credentials not configured for production"]
            else:
    ...
    def health_check(self) -> Dict[str, any]:
        ...
        overall_status = "healthy" if (jwt_working and user_service_healthy) else "unhealthy"
